      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg p-4 sm:p-6 border border-gray-200 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="description"
              placeholder="Add additional details about the result (e.g., revaluation status, important notes, deadlines)"
              value={formData.description || ''}
              onChange={(e) => setFormData({ ...formData, description: e.target.value })}
              rows={3}
              className="resize-none"
            />
            <p className="text-xs text-[#0A0A0A]/60">Optional description to provide more context</p>
          </div>
          ...
          {/* Result Link */}
          <div className="space-y-2">
            <Label htmlFor="link">Result Link (Legacy)</Label>
            <Input
              id="link"
              type="url"
            ...
            />
            <p className="text-xs text-[#0A0A0A]/60">Keep for backward compatibility</p>
          </div>

          {/* Server 1 Link */}
          <div className="space-y-2">
            <Label htmlFor="server1_link">Server 1 Link</Label>
            <Input
              id="server1_link"
              type="url"
              placeholder="https://server1.example.com/results/..."
              value={formData.server1_link || ''}
              onChange={(e) => setFormData({ ...formData, server1_link: e.target.value })}
            />
          </div>

          {/* Server 2 Link */}
          <div className="space-y-2">
            <Label htmlFor="server2_link">Server 2 Link</Label>
            <Input
              id="server2_link"
              type="url"
              placeholder="https://server2.example.com/results/..."
              value={formData.server2_link || ''}
              onChange={(e) => setFormData({ ...formData, server2_link: e.target.value })}
            />
          </div>
